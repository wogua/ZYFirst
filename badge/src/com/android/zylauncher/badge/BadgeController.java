...
public class BadgeController extends BroadcastReceiver {
    ...
    public static void drawUnreadEventIfNeed(Canvas canvas, View icon, boolean isShowingUnread) {
        ...
        if (info != null && info.unreadNum > 0) {
            ...
            Log.d(TAG, "drawUnread count: " + info.unreadNum + ", " + info.getTargetComponent());
            /// M: Draw unread text.
    ...
    public static void checkAndSetShortCutInfo(final ShortcutInfo info,final ArrayList<BadgeInfo> unreadApps) {
        ...
        String truePkg = getDualPackage(info);
        for (BadgeInfo badgeInfo : unreadApps) {
            if (componentName != null && badgeInfo.pkgName != null && badgeInfo.pkgName.equals(truePkg)
    ...
    public static String getDualPackage(final ShortcutInfo info){
        final Intent intent = info.intent;
        final ComponentName componentName = intent.getComponent();
        if(componentName!=null){
            if("com.vapp.aide.intl".equals(componentName.getPackageName())){
                String dualPkg = intent.getStringExtra("EXTRA_LAUNCH_PACKAGE");
                if(dualPkg!=null){
                    return dualPkg+LauncherBadgeProvider.BADGE_PKG_TEMP;
                }
            }
