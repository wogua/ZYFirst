...
//        setBadge("com.tencent.qqmusic",444);
    ...
    private boolean setUnreadNumberForWeChat(Context context, int count) {
        ...
        try {
            ...
            Log.d(TAG, "setUnreadNumberForWeChat exception : " + e.toString());
            e.printStackTrace();
            return false;
        }
    }
    ...
    public Bundle call(String method, String arg, Bundle extras) {
        Log.d(TAG, "call package:"+getCallingPackage());
        ...
        if ("setBadge".equals(method)) {
            if (extras == null) {
                ...
                if ("com.vapp.aide.intl".equals(getCallingPackage())) {
                    String pkg = extras.getString("badge_package");
                    if (pkg != null) {
                        bundle.putBoolean("result", setBadge(pkg + BADGE_PKG_TEMP, count));
                    } else {
                        bundle.putBoolean("result", false);
                    }
                } else {
        ...
        try {
            ...
            if (cursor == null || cursor.getCount() <= 0) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    Log.d(TAG, "reloadBadges : " + cc + ", " + pkg);
                }
                callbacks.bindWorkspaceUnreadInfo(badgeInfos);
            }
        } catch (Exception e) {
            Log.d(TAG, "reloadBadges Exception : " + e.toString());
            return;
        } finally {
            if (cursor != null) {
                cursor.close();
