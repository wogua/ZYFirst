...
public class FolderIcon extends FrameLayout implements FolderListener ,UninstallMode,UninstallMode.UninstallAnimation{
    ...
    public BubbleTextView getFolderName(){// add
        return mFolderName;
    }
    ...
    public static FolderIcon fromXml(int resId, Launcher launcher, ViewGroup group,
        ...
        icon.mFolderName.setTextColor(ColorManager.getInstance().getColors()[0]);// add for color change
        // add for change folderName begin
        String folderName = null;
        int stringId = 0;
        if("".equals(folderInfo.title)) {
            folderName = launcher.getResources().getString(R.string.folder_hint_text);
        }else {
            folderName = checkDefaultFolderName((String) folderInfo.title, launcher.getResources());
            if(folderName==null){
                folderName = (String) folderInfo.title;
            }
        }
        // add for change folderName end
...
//UninstallMode add by  end
    ...
    public static class PreviewBackground {
        ...
        public void setup(DisplayMetrics dm, DeviceProfile grid, View invalidateDelegate,
            ...
            this.previewSize = folderBgSize;/*previewSize - 2 * previewPadding*/// change
...
//        }
    ...
    public void onAdd(ShortcutInfo item) {
        updateFolderUnreadNum();// add for unread
        invalidate();
        requestLayout();
    }
    ...
    /**
     *  add for unread ,called from setBadge
     */
    public void updateFolderUnreadNum(String packageName, int count, String shortcutCustomId) {
        ...
        for (int i = 0; i < contentsCount; i++) {
            shortcutInfo = contents.get(i);
            componentName = shortcutInfo.intent.getComponent();
            final String pkgName = BadgeController.getDualPackage(shortcutInfo);
            boolean isCurrentShortCut = shortcutInfo.shortcutCustomId == null || shortcutInfo.shortcutCustomId.equals(shortcutCustomId);
            if (componentName != null && packageName.equals(pkgName) && isCurrentShortCut) {
    ...
    public static String checkDefaultFolderName(String charSequence, Resources res) {
        String[] strings = charSequence.split(":");
        if (strings != null && strings.length == 3&&res!=null) {
            String packageName = strings[0];
            String resourceName = strings[2];
            int stringId = res.getIdentifier(resourceName, "string", packageName);
            if(stringId!=0){
