...
//        }
        ...
        InstallShortcutReceiver.disableAndFlushInstallQueue(InstallShortcutReceiver.FLAG_DRAG_AND_DROP,getContext());
...
//        return numCustomPages();
    ...
    public void stripEmptyScreensForHideApp() {
        if (mLauncher.isWorkspaceLoading()) {
            // Don't strip empty screens if the workspace is still loading.
            // This is dangerous and can result in data loss.
            return;
                     ...
                     //  add for hotseat icon center start
                       if(toHotseat){
                            if(!dragFromHotseat(d.dragInfo)) {
                                ...
                                if(mLauncher.isLandscape) {
                                    addInHotseat(cell, container, screenId, 0, index,
        ...
        for (ItemInfo info : itemInfos) {
            ...
            if (UninstallShortcutReceiver.isDualIntent(info.getIntent())) {
                ...
                if (resolveInfos == null || resolveInfos.size() == 0) {
                    mLauncher.removeItem(children.get(info),info,true);
                }
            }
        }
    }
...
//        Log.d("","refreshviewCache "+log);
    ...
    public void refreshSourceViewCache() {
        if (sourceIndex >= 0) {
            refreshviewCache(sourceIndex, null, null, false);
        }
    }
        ...
        for (ShortcutAndWidgetContainer layout : childrenLayouts) {
            ...
            for (int j = 0; j < childCount; j++) {
                ...
                if (tag instanceof ShortcutInfo) {
                    ...
                    final String pkgName = BadgeController.getDualPackage(info);
...
//            openFolder.updateContentUnreadNum();
    ...
    public void removeDualShortcut(ItemInfo itemInfo) {
        ...
        componentNames.add(itemInfo.getIntent().getStringExtra("EXTRA_LAUNCH_PACKAGE"));
        if(componentNames.size()==0){
             return;
         }
        final ItemInfoMatcher matcher = ItemInfoMatcher.ofPackagesDual(componentNames, itemInfo.user);
        boolean removeHoeseatItem = false;
        ArrayList<CellLayout> cellLayouts = getWorkspaceAndHotseatCellLayouts();
        for (final CellLayout layoutParent: cellLayouts) {
            final ViewGroup layout = layoutParent.getShortcutsAndWidgets();
