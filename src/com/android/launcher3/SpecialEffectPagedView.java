...
public class SpecialEffectPagedView extends SlidePagedView {
    ...
    protected void drawScreen(Canvas canvas, int screen, long drawingTime) {
        ...
        switch (mPageAnimType) {
            ...
            case Type.CASCADE:
                ...
                break;
            case Type.DEPTH_FOCUS:
                if (mLauncher.getWorkspace().inScaleState())
                    drawChild = depthFocusEditMode(child, screen, (float) percentage);
                else
                    drawChild = depthFocus(child, screen, (float) percentage);
                break;
            default:
                break;
        }
    ...
    private boolean boxOut(Canvas canvas, int screen, float percentage) {
        ...
        if (angle >= 0) {
            centerX = childW * 5 / 2 + pageOffsetX + (int) (percentage * childW / 30);
            ...
            centerX = childW * 3 / 2 + pageOffsetX - (int) (percentage * childW / 20);
