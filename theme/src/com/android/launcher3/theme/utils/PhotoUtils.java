 ...
 */
    ...
    public static Bitmap compositeByBitmap(Bitmap srcBmp,Drawable mask,Drawable background,Drawable zoomTemplate,boolean isWidget) {
        ...
        Log.d("IconManager2","debug 1 compositeByBitmap  zmBmp width = " + zmBmp.getWidth() + ", srcBmp width = " + srcBmp.getWidth());
        canvas.restore();
        ...
        Log.d("IconManager2","debug 2 compositeByBitmap  zmBmp width = " + zmBmp.getWidth() + ", maskBmp width = " + maskBmp.getWidth());
        canvas2.setDrawFilter(new PaintFlagsDrawFilter(Paint.DITHER_FLAG,
                Paint.FILTER_BITMAP_FLAG));
        canvas2.restore();
        paint.setXfermode(null);
        ...
        Log.d("IconManager2","debug 3 compositeByBitmap  maskBmp width = " + maskBmp.getWidth() + ", bgBmp width = " + bgBmp.getWidth());
    ...
    //加双开水印
    public static Bitmap compositeLBEBitmap(Bitmap source,Bitmap watermark) {
        if (source == null || watermark == null) return source;
        if (watermark.getWidth() > source.getWidth()) {
            watermark = zoom(watermark, source.getWidth(), source.getHeight());
        }
        int w = source.getWidth() - watermark.getWidth();
        int h = source.getHeight() - watermark.getHeight();
        ...
        canvas.drawBitmap(watermark, w, h, null);
        return bitmap;
