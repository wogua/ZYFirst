...
public abstract class IconGetterAbsImpl implements IIconGetter{
    ...
    protected HashMap<String, Float> mLabel_dimens = new HashMap<String,Float>();
    ...
    //获取双开水印图案
    protected Bitmap getLBEWaterMark(){
        Context context;
        try {
            context = createThemeContext(mContext);
    ...
    public Bitmap getLBEBitmap(String packageName) {
        Bitmap result = getIcon(packageName, null);
        if (result != null) {
            result = PhotoUtils.compositeLBEBitmap(result, getLBEWaterMark());
        }
        return result;
    }

    private Drawable getIconFromApk(String pkg, String cls, UserHandle user) {
//        Log.d("IconManager","getIconFromApk pkg:"+pkg+", cls:"+cls +"\n"+Log.getStackTraceString(new Throwable()));
